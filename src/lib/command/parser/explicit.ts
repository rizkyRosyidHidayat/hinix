...
export function parseExplicitCommand(
	input: string,
	...
	const match = input
		.trim()
		.match(
			/^(todo|schedule|habit|budget|note)s?\s+(add|create|list|show|update|delete|remove)\s*(.*)$/i
		);
	...
	const action: CommandAction =
		rawAction === 'add' || rawAction === 'create'
			? 'create'
			: rawAction === 'show' || rawAction === 'list'
				? 'list'
				: rawAction === 'remove'
					? 'delete'
					: (rawAction as CommandAction);
