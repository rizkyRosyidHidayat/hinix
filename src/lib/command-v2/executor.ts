...
class AppCommandExecutor implements CommandExecutor {
	...
	async execute(command: ParsedCommand): Promise<CommandResult> {
		if (command.status !== 'parsed' || !command.domain || !command.action) {
			throw new Error('Command cannot be executed until it is successfully parsed.');
		}

		if (command.needsConfirmation) {
			return { type: 'text', output: 'Command requires confirmation to execute.' };
		}
		...
		switch (domain) {
			case 'todo':
				return this.executeTodo(action, entities);
			case 'schedule':
				return this.executeSchedule(action, entities);
			case 'habit':
				return this.executeHabit(action, entities);
			case 'budget':
				return this.executeBudget(action, entities);
			case 'note':
				return this.executeNote(action, entities);
			default:
