...
export const habitsCommand: CommandDefinition = {
	...
	subcommands: [
		...
		{
			name: 'list',
			description: "List habits and today's completion state",
			usage: 'list',
			example: 'list'
		},
		{
	...
	async execute(args: string[], context: CommandContext) {
		...
		switch (subCommand) {
			case 'add': {
				...
				if (!name)
					return { type: 'error', output: 'Habit name is required. Usage: habits add <name>' };
			...
			case 'done': {
				...
				try {
					...
					if (e instanceof Error && e.message.includes('not found')) {
						return {
							type: 'error',
							output: `Habit "${habitName}" not found.\n\nTry:\n  list\n  add ${habitName}`
						};
					}
					return { type: 'error', output: e instanceof Error ? e.message : 'Unknown error' };
				}
			...
			default:
				return {
					type: 'error',
					output: `Unknown subcommand: ${subCommand}. Type "help" for available commands.`
				};
		}
	}
};
