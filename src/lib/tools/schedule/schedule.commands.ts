  ...
  subcommands: [
    ...
    },
    {
      ...
      flags: [
        ...
        {
          name: 'note-id',
          description: 'Link an existing note', usage: '--note-id <id>',
          example: '--note-id abc123',
          suggest: async (input: string, context: CommandContext) => {
            const service = new NotesService(context.repositories.notes);
            const notes = await service.list();
        ...
        { name: 'unlink-note', description: 'Remove linked note from this event', usage: '--unlink-note' }
      ],
      suggest: async (input: string, context: CommandContext) => {
        const service = new ScheduleService(context.repositories.schedule, context.repositories.habits);
    ...
    switch (subCommand) {
      case 'add': {
        ...
        if (noteIdIndex !== -1 && noteIdIndex + 1 < args.length) {
          ...
          if (!note) {
            return { type: 'error', output: `Note with ID starting with "${rawNoteId}" not found.` };
          }
          linkedNoteId = note.id;
      ...
      case 'update': {
        ...
        try {
          ...
          if (!hasChanges && !noteUnlinked) {
            return { type: 'error', output: 'Nothing to update. Provide --time, --date, --note-id, or --unlink-note.' };
          }
