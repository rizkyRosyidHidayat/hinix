...
export const todoCommand: CommandDefinition = {
	...
	usage:
		'todo [add <title> | list | view <id> | update <id> | done <id> | undo <id> | delete <id>]',
	subcommands: [
		{
			name: 'add',
				...
				if (deadlineIndex !== -1 && deadlineIndex + 1 < args.length) {
					...
					if (part1.includes(' ')) {
						...
						if (timeRegex.test(t) && dateRegex.test(d)) {
							...
							return {
								type: 'error',
								output: 'Invalid --deadline format. Format: <HH:MM> [DD-MM-YYYY]'
							};
						}
			...
			case 'update': {
				...
				if (!id)
					return {
						type: 'error',
						output:
							'ID is required. Usage: todo update <id> [--deadline <HH:MM> [DD-MM-YYYY]] [--description <value>] [--title <value>]'
					};
